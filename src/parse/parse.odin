...
parse :: proc (path: string) -> (result: ast.module) {
	if !strings.ends_with(path, ".nya") {
		fmt.eprintfln("\e[1;31merror\e[0m " + "file %s does not have a .nya extension", path)
		os.exit(1)
	}
	code: string
	if path == "-" {
		buf := make([dynamic]u8, 1024)
		i := 0
		for {
